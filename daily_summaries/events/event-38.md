---
event_id: 38
status: "ACTIVE"
event_type: "PARTNERSHIP"
event_date: null
impact_direction: "UNCERTAIN"
impact_strength: 2
importance_score: 35
confidence: 0.6
time_horizon: "UNCERTAIN"
companies:
  - "Boeing Co/The [company_id: 54]"
topics:
  - "Government contracts"
  - "Launch services"
updated_at: "2026-08-23T23:46:53.51388+00:00"
---

# Boeing still working with NASA to schedule next Starliner flight

Boeing is working with NASA to determine the timing for the next flight of its CST-100 Starliner commercial crew vehicle, which would fly as a cargo mission to the International Space Station.

## Industry implication

The scheduling uncertainty means NASA and Boeing have not finalized when Starliner will next support ISS cargo operations; this could affect near-term cargo planning and Boeing's commercial crew program milestone tracking, but the summary does not indicate a firm delay or cancellation.

## Key facts

- Boeing is still working with NASA to determine when to fly its CST-100 Starliner commercial crew vehicle on a cargo mission to the International Space Station.

## Risks

- Timing for the next Starliner cargo flight is unresolved, creating uncertainty for NASA cargo scheduling and Boeing operational planning.
- Uncertainty in scheduling could lead to downstream impacts on ISS resupply manifests or reliance on alternate cargo providers if delays occur.

## Sources

- [Boeing still working with NASA to schedule next Starliner flight](https://spacenews.com/boeing-still-working-with-nasa-to-schedule-next-starliner-flight/) — 2026-07-29 10:24 KST

## Related events

- [[event-5|NASA’s B777 Gets New Coat of Paint]]
- [[event-39|SpaceX wins $1.6 billion in launch orders for military satellite networks]]
- [[event-40|Rocket Lab to open Alaska launch site under $266 million Space Force deal]]
- [[event-43|Nancy Grace Roman Space Telescope Launches Aboard SpaceX Falcon Heavy]]
- [[event-50|BOEING CO files 8-K]]
- [[event-1|NRO to expand use of HawkEye 360’s satellite intelligence]]
- [[event-2|New report takes closer look at the Space Force spending surge]]
- [[event-8|NASA Selects Companies to Provide Payload Processing Services]]
- [[event-11|NOAA awards radio‑occultation contracts to Spire and PlanetiQ; extends Spire HyMS evaluation contract]]
- [[event-12|Portal Space Systems to include rideshare payloads on Falcon 9 launch of Supernova]]
